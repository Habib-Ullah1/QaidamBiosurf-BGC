...
OUTDIR=$PROJECT/biosurfactant/14_deseq2
mkdir -p $OUTDIR
...
# Load coverage for all 4 samples
...
# Load biosurfactant HMMER hit contigs
...
for(s in samples){
  ...
  df <- read.table(f, header=TRUE, sep="\t", stringsAsFactors=FALSE)
  # Extract contig ID from orf_id (remove last _N)
  contigs <- sub("_[0-9]+$", "", df$orf_id)
  bs_contigs <- union(bs_contigs, contigs)
}
cat(sprintf("  Unique biosurfactant contigs: %d\n", length(bs_contigs)))

# Filter coverage matrix to biosurfactant contigs only
bs_mat <- cov_mat[rownames(cov_mat) %in% bs_contigs, ]
cat(sprintf("  Biosurfactant contigs in coverage matrix: %d\n", nrow(bs_mat)))

# Convert to integer counts (DESeq2 requires integers)
# Multiply by 1000 and round to get pseudo-counts
count_mat <- round(bs_mat * 1000)
count_mat <- count_mat[rowSums(count_mat) > 0, ]
cat(sprintf("  Non-zero contigs after filtering: %d\n", nrow(count_mat)))

# Create sample metadata
col_data <- data.frame(
  sample = samples,
  season = c("summer","summer","summer","winter"),
  row.names = samples
)
...
# Get results: winter vs summer (positive = higher in winter)
res <- results(dds,
...
sig_summer <- sig[sig$log2FoldChange < 0, ]  # higher in summer
sig_winter  <- sig[sig$log2FoldChange > 0, ]  # higher in winter

cat(sprintf("\n=== DESeq2 Results ===\n"))
cat(sprintf("Total contigs tested:        %d\n", nrow(res_df)))
cat(sprintf("Significant (padj<0.05):     %d\n", nrow(sig)))
cat(sprintf("  Higher in summer:          %d\n", nrow(sig_summer)))
cat(sprintf("  Higher in winter:          %d\n", nrow(sig_winter)))
...
cat("\n=== Top 20 summer-enriched biosurfactant contigs ===\n")
print(head(sig_summer[,c("contig","log2FoldChange","padj","baseMean")], 20))

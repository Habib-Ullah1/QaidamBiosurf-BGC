...
# ============================================================
# SUPP FIGURE S1 — already good, reproduce as-is
# ============================================================
cat("Building Figure S1...\n")
...
contig_data <- data.frame(
  ...
  LengthBin=factor(rep(c("500-999","1000-1999","2000-4999",
                   ...
                   levels=c("500-999","1000-1999","2000-4999",
                             "5000-9999",">=10000")),
  Count=c(926891,245598,69972,8984,2755,
          438138,95129,23861,2823,751,
          711556,184288,56040,9055,3084,
          167152,35993,9362,1521,474)
)
...
bin_colors <- c("500-999"="#4292C6","1000-1999"="#2171B5",
                "2000-4999"="#08519C","5000-9999"="#F16913",
                ">=10000"="#D94801")

pS1a <- ggplot(contig_pct,aes(x=Sample,y=Count/1000,fill=LengthBin)) +
...
pS1b <- ggplot(contig_pct,aes(x=Sample,y=Pct,fill=LengthBin)) +
  geom_bar(stat="identity",width=0.7,color="white",linewidth=0.2) +
  scale_fill_manual(values=bin_colors,name="Contig length (bp)") +
...
n50_data <- data.frame(
  ...
  N50=c(1010,891,1053,925),TotalMb=c(1259,515,1000,205)
)

pS1c <- ggplot(n50_data,aes(x=Sample,y=N50,fill=Season)) +
  geom_bar(stat="identity",width=0.6,color="white",linewidth=0.3) +
  geom_text(aes(label=paste0(N50," bp")),vjust=-0.4,size=3,fontface="bold") +
  scale_fill_manual(values=sc,name="Season") +
  scale_y_continuous(expand=expansion(mult=c(0,0.15))) +
  labs(x=NULL,y="N50 (bp)",title="C. Assembly N50 per sample") +
  pub_theme + theme(legend.position="none")

pS1d <- ggplot(n50_data,aes(x=Sample,y=TotalMb,fill=Season)) +
  geom_bar(stat="identity",width=0.6,color="white",linewidth=0.3) +
  geom_text(aes(label=paste0(TotalMb," Mb")),vjust=-0.4,size=3,fontface="bold") +
  scale_fill_manual(values=sc,name="Season") +
  scale_y_continuous(expand=expansion(mult=c(0,0.15))) +
  labs(x=NULL,y="Total assembly length (Mb)",
       title="D. Total assembly size per sample") +
  pub_theme + theme(legend.position="bottom",legend.direction="horizontal")

figS1 <- (pS1a|pS1b)/(pS1c|pS1d) + plot_layout(heights=c(1.5,1))
...
# ============================================================
# SUPP FIGURE S2 — Fix: legend cut, "F novelty" title, B labels
# ============================================================
cat("Building Figure S2...\n")
...
gcf_sizes <- data.frame(
  Size_Category=factor(
    ...
    levels=c("Singleton\n(1 BGC)","Small\n(2-3 BGCs)",
              "Medium\n(4-9 BGCs)","Large\n(>=10 BGCs)")),
  GCF_Count=c(376,178,30,6),Pct=c(63.7,30.2,5.1,1.0)
)

pS2a <- ggplot(gcf_sizes,aes(x=Size_Category,y=GCF_Count,fill=Size_Category)) +
  ...
  scale_fill_manual(values=c("#C0392B","#E67E22","#2980B9","#27AE60"),guide="none") +
  scale_y_continuous(expand=expansion(mult=c(0,0.2))) +
  labs(x="GCF size (number of member BGCs)",y="Number of GCFs",
       title="A. GCF size distribution (n=590 GCFs, c=0.50)") +
  pub_theme

gcf_class <- data.frame(
  Class=rep(c("NRPS","PKS other","RiPPs","Terpene","Others","PKS I"),3),
  # FIX: shortened season labels to prevent legend cut
  Season=factor(rep(c("Summer-only","Cross-season","Winter-only"),each=6),
                levels=c("Summer-only","Cross-season","Winter-only")),
  Count=c(22,22,75,58,44,5, 3,11,17,51,12,2, 35,31,52,47,71,3)
)

season_col <- c("Summer-only"="#E8593C","Cross-season"="#7D9B76",
                "Winter-only"="#3B8BD4")

pS2b <- ggplot(gcf_class,aes(x=Class,y=Count,fill=Season)) +
  ...
  theme(legend.position="bottom",legend.direction="horizontal",
        # Extra right margin to prevent legend cut
        plot.margin=margin(5,25,5,5))

novelty_class <- data.frame(
  Class=factor(c("NRPS","PKS other","RiPPs","Terpene","Others","PKS I"),
               levels=c("NRPS","PKS other","RiPPs","Terpene","Others","PKS I")),
...
nov_long <- novelty_class %>%
  ...
  mutate(Category=factor(
    recode(Category,Singleton="Singleton (novel)",
           Multi_novel="Multi-member (novel)",
           MIBiG_related="MIBiG-related"),
    levels=c("MIBiG-related","Multi-member (novel)","Singleton (novel)")))

pS2c <- ggplot(nov_long,aes(x=Class,y=Count,fill=Category)) +
  geom_bar(stat="identity",width=0.7,color="white",linewidth=0.2) +
  scale_fill_manual(
    values=c("MIBiG-related"="#27AE60","Multi-member (novel)"="#E67E22",
             "Singleton (novel)"="#C0392B"),
    # FIX: "F novelty" was because name was not set — set it explicitly
    name="GCF novelty") +
  scale_y_continuous(expand=expansion(mult=c(0,0.08))) +
  labs(x="BGC class",y="Number of GCFs",
       # FIX: correct title
       title="C. GCF novelty composition per BGC class") +
  pub_theme +
  theme(legend.position="bottom",legend.direction="horizontal",
        plot.margin=margin(5,25,5,5))
...
cutoff_data <- data.frame(
  ...
  Singletons=c(312,376,441),MultiMember=c(208,214,182)
)
cutoff_long <- cutoff_data %>%
  pivot_longer(cols=c(Singletons,MultiMember),
               names_to="Type",values_to="Count")

pS2d <- ggplot(cutoff_long,aes(x=Cutoff,y=Count,fill=Type)) +
  geom_bar(stat="identity",width=0.6,color="white",linewidth=0.3) +
  scale_fill_manual(
    values=c("Singletons"="#C0392B","MultiMember"="#3B8BD4"),
    labels=c("Multi-member GCFs","Singleton GCFs"),name=NULL) +
  scale_y_continuous(expand=expansion(mult=c(0,0.1))) +
  labs(x="BiG-SCAPE cosine similarity cutoff",y="Number of GCFs",
       title="D. GCF count at three similarity cutoffs") +
  pub_theme +
  theme(legend.position="bottom",legend.direction="horizontal")

# FIX: wider figure (190mm) to accommodate legends
figS2 <- (pS2a|pS2b)/(pS2c|pS2d)
ggsave(file.path(OUTDIR,"FigureS2_bigscape_network.pdf"),
       figS2,width=190,height=160,units="mm",dpi=300)
ggsave(file.path(OUTDIR,"FigureS2_bigscape_network.png"),
       figS2,width=190,height=160,units="mm",dpi=300)
cat("S2 done.\n")

# ============================================================
# SUPP FIGURE S3 — Fix: wider figure, fix subtitle cuts
# ============================================================
cat("Building Figure S3...\n")
...
pS3a <- ggplot(deseq_all,
  ...
  scale_color_manual(
    values=c("Summer enriched"="#E8593C","Winter enriched"="#3B8BD4",
  ...
  labs(x=expression(log[10]*"(mean normalised coverage + 1)"),
       ...
       # FIX: shorter subtitle that fits
       subtitle="Dashed lines at log2FC = +/-1 (biological significance threshold)") +
  guides(color=guide_legend(override.aes=list(size=2.5,alpha=1),nrow=3)) +
  pub_theme +
  theme(legend.position=c(0.72,0.12),
        legend.background=element_rect(fill="white",color="grey80",linewidth=0.3),
        legend.text=element_text(size=7))

pS3b <- ggplot(deseq_all[!is.na(deseq_all$pvalue),],aes(x=pvalue)) +
  geom_histogram(bins=40,fill="#2980B9",color="white",linewidth=0.2) +
  ...
  labs(x="Raw p-value",y="Number of contigs",
       ...
       subtitle="Enrichment near 0 indicates valid statistical test") +
  pub_theme
...
pS3c <- ggplot(sig_only,aes(x=log2FoldChange,fill=category)) +
  ...
  labs(x=expression(log[2]*" Fold Change"),y="Number of significant contigs",
       title="C. log2FC distribution of significant contigs",
       subtitle=paste0("n=",nrow(sig_only)," significant contigs (padj<0.05)")) +
  annotate("text",x=-20,y=Inf,label=paste0("Summer\nn=",n_s),
           color="#E8593C",vjust=1.2,size=3,fontface="bold") +
  annotate("text",x=15,y=Inf,label=paste0("Winter\nn=",n_w),
           color="#3B8BD4",vjust=1.2,size=3,fontface="bold") +
  pub_theme

# FIX: shorter subtitle and more bottom margin for D
pS3d <- ggplot(sig_only,
               aes(x=category,y=log10(baseMean+1),fill=category)) +
  geom_boxplot(width=0.5,color="black",
               outlier.size=0.5,outlier.alpha=0.3) +
  scale_fill_manual(
    values=c("Summer enriched"="#E8593C","Winter enriched"="#3B8BD4"),
    guide="none") +
  labs(x=NULL,y=expression(log[10]*"(mean coverage + 1)"),
       title="D. Mean coverage by enrichment group",
       # FIX: shorter subtitle
       subtitle="Higher baseMean = higher abundance across all samples") +
  pub_theme +
  theme(axis.text.x=element_text(size=8),
        plot.margin=margin(5,15,15,5))

# FIX: wider figure (190mm)
figS3 <- (pS3a|pS3b)/(pS3c|pS3d)
ggsave(file.path(OUTDIR,"FigureS3_deseq2_supplement.pdf"),
       figS3,width=190,height=160,units="mm",dpi=300)
ggsave(file.path(OUTDIR,"FigureS3_deseq2_supplement.png"),
       figS3,width=190,height=160,units="mm",dpi=300)
cat("S3 done.\n")

# ============================================================
# SUPP FIGURE S4 — Fix: more left margin for phylum names
# ============================================================
cat("Building Figure S4...\n")
...
phylum_colors2 <- c(
  "Pseudomonadota"="#E74C3C","Chloroflexota"="#27AE60",
  "Halobacteriota"="#9B59B6","Nanohaloarchaeota"="#8E44AD",
  "Bacillota_A"="#F39C12","Bacteroidota"="#3498DB",
  "Actinomycetota"="#1ABC9C","Gemmatimonadota"="#E67E22",
  "Myxococcota"="#C0392B","Deinococcota"="#7F8C8D",
  "Patescibacteria"="#BDC3C7"
)

phylum_summary <- mag_tax %>% count(Phylum,Domain) %>% arrange(Domain,desc(n))
phylum_summary$Phylum <- factor(phylum_summary$Phylum,levels=phylum_summary$Phylum)

pS4a <- ggplot(phylum_summary,aes(x=n,y=reorder(Phylum,n),fill=Phylum)) +
  geom_bar(stat="identity",width=0.7,color="white",linewidth=0.3) +
  ...
  pub_theme +
  # FIX: extra left margin so phylum names are not cut
  theme(plot.margin=margin(5,15,5,20))

# FIX: extra left margin for B phylum labels
pS4b <- ggplot(mag_tax,
               aes(x=reorder(Phylum,Completeness,median),
                   y=Completeness,fill=Phylum)) +
  geom_boxplot(width=0.6,color="black",outlier.size=1) +
  geom_hline(yintercept=c(50,90),linetype="dashed",
             color=c("#F39C12","#27AE60"),linewidth=0.5) +
  scale_fill_manual(values=phylum_colors2,guide="none") +
  scale_y_continuous(limits=c(48,103),breaks=seq(50,100,10)) +
  labs(x=NULL,y="Completeness (%)",
       title="B. Completeness distribution by phylum") +
  coord_flip() +
  pub_theme +
  # FIX: generous left margin for long phylum names after coord_flip
  theme(plot.margin=margin(5,10,5,25),
        axis.text.y=element_text(size=7.5))

pS4c <- ggplot(mag_tax,aes(x=Domain,fill=Season)) +
  ...
  geom_text(stat="count",aes(label=after_stat(count),group=Season),
...
# FIX: wider figure (200mm) to give room to phylum labels
figS4 <- (pS4a|pS4b)/(pS4c|pS4d) + plot_layout(heights=c(1.2,1))
ggsave(file.path(OUTDIR,"FigureS4_MAG_phylogeny.pdf"),
       figS4,width=200,height=200,units="mm",dpi=300)
ggsave(file.path(OUTDIR,"FigureS4_MAG_phylogeny.png"),
       figS4,width=200,height=200,units="mm",dpi=300)
cat("S4 done.\n")

# ============================================================
# SUPP FIGURE S5 — Fix: panel B title cut, subtitle cuts
# ============================================================
cat("Building Figure S5...\n")

halo_genes <- data.frame(
  Category=c("FA biosynthesis","NRPS/PKS\nrelated","Acyltransferase",
             "Lipoprotein\nsecretion","ABC\ntransporter","Efflux/\nresistance",
             "Phospholipid\nbiosyn.","Quorum\nsensing","Two-component\nreg.",
             "Other\nbiosurfactant"),
...
flagship_genes$MAG <- factor(flagship_genes$MAG,
  levels=c("Halomonas_B\n(H2, winter)","Moraxella_A\n(H6, winter)"))
flagship_genes$Category <- factor(flagship_genes$Category,
  levels=c("FA biosynthesis","NRPS/PKS\nrelated","Acyltransferase",
            "Lipoprotein\nsecretion","ABC\ntransporter","Efflux/\nresistance",
            "Phospholipid\nbiosyn.","Quorum\nsensing","Two-component\nreg.",
            "Other\nbiosurfactant"))

mag_colors3 <- c("Halomonas_B\n(H2, winter)"="#3B8BD4",
                 "Moraxella_A\n(H6, winter)"="#9B59B6")
...
pS5a <- ggplot(flagship_genes,aes(x=Category,y=Count,fill=MAG)) +
  ...
  pub_theme +
  theme(legend.position="bottom",legend.direction="horizontal",
        axis.text.x=element_text(size=7,angle=45,hjust=1,vjust=1,lineheight=0.85))
...
key_long <- key_genes_df %>%
  pivot_longer(cols=c(Halomonas_B,Moraxella_A),
               names_to="MAG",values_to="Present") %>%
  mutate(
    # FIX: shorter MAG labels that fit without cut
    MAG=recode(MAG,Halomonas_B="Halomonas_B",Moraxella_A="Moraxella_A"),
...
pS5b <- ggplot(key_long,aes(x=MAG,y=Gene,fill=Fill)) +
  ...
  labs(x=NULL,y=NULL,
       # FIX: shorter title that fits in panel
       title="B. Key biosurfactant genes\nin flagship MAGs") +
  pub_theme +
...
genome_stats <- data.frame(
  ...
  Total_genes=c(4321,2497),Named_genes=c(2734,1539),
...
gs_long <- genome_stats %>%
  pivot_longer(cols=c(Total_genes,Named_genes,Biosurfactant_genes),
               names_to="Category",values_to="Count") %>%
  mutate(Category=factor(
    recode(Category,Total_genes="Total genes",
...
pS5c <- ggplot(gs_long,aes(x=Category,y=Count,fill=MAG)) +
  ...
  geom_text(aes(label=Count,group=MAG),
            ...
            vjust=-0.4,size=3,fontface="bold") +
  scale_fill_manual(
    values=c("Halomonas_B"="#3B8BD4","Moraxella_A"="#9B59B6"),name=NULL) +
...
# FIX: wider figure (190mm), taller to fit all panels

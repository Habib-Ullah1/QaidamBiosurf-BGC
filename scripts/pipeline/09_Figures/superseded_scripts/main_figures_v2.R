...
# =============================================================================
# FIGURE 4 — Convergent multi-evidence functional analysis
# Panel order (new): A = multi-evidence bubble matrix
#                    B = C-domain phylogenetic classes
#                    C = NRPS domain architecture (8 GCFs)
# Layout: A and B share top row (A wider than B); C spans full width below.
# =============================================================================
cat("Building Figure 4...\n")

# ---- Panel A (was C) — Multi-evidence bubble matrix ------------------------
...
p4a <- ggplot(evidence_df,
  ...
  labs(x = NULL, y = NULL,
       title = "A   Convergent multi-evidence pathway detection") +
  pub_theme +
...
# ---- Panel B (unchanged) — C-domain phylogenetic classes -------------------
cdom_levels <- rev(c(
...
# ---- Panel C (was A) — NRPS domain architecture (8 GCFs), full-width row ---
# Keep season facets as rows (original orientation), but assembled into a
# full-width bottom row. Long GCF labels have room; tiles stay legible.
gcf_levels <- c(
  "GCF 226 (C3+C6) | Summer",
  "GCF 521 (C3+C6) | Summer",
  "GCF 993 (C6+H2) | Cross-season",
  "GCF 429 (C3+H2) | Cross-season",
  "GCF 756 (C6+H2) | Cross-season",
  "GCF 1429 (H2+H6) | Winter",
  "GCF 1253 (H2+H6) | Winter",
  "GCF 841 (H2+H6) | Winter")
...
# Full-width bottom row: facet by season as COLUMNS so the limited content
# (4 modules) spreads horizontally instead of leaving empty white space.
# Each season column gets its own GCFs (free_y) and the season label sits
# above each block, matching the original visual logic.
p4c <- ggplot(nrps_gcfs,
  ...
  facet_wrap(~ SeasonCat, scales = "free_y", nrow = 1) +
  labs(y = NULL,
       title = "C   NRPS domain architecture (8 GCFs)") +
...
# ---- Assemble Figure 4 ------------------------------------------------------
# Top row: A (bubble matrix, wider) + B (C-domain bars).
# A:B ratio ~1.35:1 because A has 5 columns of dense bubbles vs. B's single
# horizontal bar chart that's already compact.
# Bottom: C spans the full width of the figure.
top4 <- (p4a + p4b) + plot_layout(widths = c(1.35, 1))
fig4 <- top4 / p4c + plot_layout(heights = c(1, 1.05)) &
  theme(plot.margin = margin(4, 8, 4, 6))

ggsave(file.path(OUTDIR, "Figure4_functional_evidence.pdf"),
       fig4, width = 190, height = 215, units = "mm",
       device = PDF_DEVICE)
ggsave(file.path(OUTDIR, "Figure4_functional_evidence.png"),
       fig4, width = 190, height = 215, units = "mm", dpi = 600)
cat("Figure 4 done.\n")
